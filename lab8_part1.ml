...
module MakeInterval (Endpoint : ORDERED_TYPE) = 
  struct
    ...
    let contains (intvl : interval) (x : Endpoint.t) : bool =
      match intvl with
      | Empty -> false
      | Interval (y,z) -> (compare x y >= 0 && compare x z <= 0) 
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      ...
      |Interval (l1, h1), Interval (l2, h2) -> 
                          if contains intvl1 l2 && contains intvl1 h2 then intvl2
                          else if contains intvl2 l1 && contains intvl2 h1 then intvl1
                          else if contains intvl1 l2 && not (contains intvl1 h2) then create l2 h1
                          else if not(contains intvl1 l2) && contains intvl1 h2 then create l1 h2
                          else if not(contains intvl1 l2) && not(contains intvl1 h2) then Empty
                          else Empty *)
    end ;;

    (*the better way: choose the larger of lows and the smaller of the highs and then call create*)
...
module MakeBestInterval (Endpoint: ORDERED_TYPE): 
  ...
  struct    
    ...
    type interval = 
    | Interval of endpoint * endpoint                    
    | Empty    (* create low high -- Returns a new interval covering `low` to       
                `high` inclusive. If `low` > `high`, then the interval is       
                empty. *)    
    let create (low : endpoint) (high : endpoint) : interval =      
        if Endpoint.compare low high > 0 then Empty      
        else Interval (low, high) (* is_empty intvl -- Returns true if and only if the `intvl` is       
                                  empty *)    
        let is_empty (intvl : interval) : bool =      
          match intvl with      
          | Empty -> true      
          | Interval _ -> false    (* contains intvl x -- Returns true if and only if the value `x`       
          is contained within `intvl` *)    
        let contains (intvl : interval) (x : Endpoint.t) : bool =      
          match intvl with      
          | Empty -> false      
          | Interval (low, high) ->         
            Endpoint.compare x low >= 0         
            && Endpoint.compare x high <= 0    (* intersect intvl1 intvl2 -- Returns the intersection of the two       
            input intervals *)    
        let intersect (intvl1 : interval) (intvl2 : interval) : interval =      
          let ordered x y = if Endpoint.compare x y <= 0 then x, y else y, x in      
            match intvl1, intvl2 with      
